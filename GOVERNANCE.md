...
|---|---|---|---|
| The published contract is implemented in Python: extraction, account binding, numeric roles, units, the unrounded comparison, negation, ratios, the totals tie, the threshold policy and the four statuses | `second_pass/checker.py` | A firm has to be able to script these checks over a close on a ledger in an accepted layout. A check that exists only as a paste into a web page cannot be scripted, and a second implementation that drifts from the first is worse than none | No scored figure moves. **Corrected 13 September 2026:** this row first said the two agreed byte for byte on fifty-nine fixtures. The fixture files then differed at T18 and T18b and the command line crashed on one of the third review's inputs. After the repair both run one file of 212 inputs and agree on every compared field for those inputs, as `CONTRACT-DIVERGENCE.md` sets out |
